...
The raw stream is `~/.local/state/ups-battery-monitor/telemetry.jsonl`. Its exact eight-field
...
`history.jsonl` is derived state: episode summaries, IR observations, and model-update

...
def test_motd_capacity_displays(model_json_with_capacity):
    ...
    assert "6.95Ah" in result.stdout or "6.95" in result.stdout, (
        f"Measured capacity not found in output: {result.stdout}"
    )
    assert "7.2Ah" in result.stdout or "7.2" in result.stdout, (
        f"Rated capacity not found in output: {result.stdout}"
    )
    assert "3/3 samples" in result.stdout or "3/3" in result.stdout, (
        f"Sample count format not found in output: {result.stdout}"
    )

    # Assert: Status badge is present (LOCKED expected since CoV should be < 0.10)
    assert (
        "LOCKED" in result.stdout or "MEASURING" in result.stdout or "UNKNOWN" in result.stdout
...
def test_motd_handles_empty_estimates(model_json_with_capacity):
    ...
    assert result.returncode == 0, (
        f"MOTD script should not crash on empty estimates: {result.stderr}"
    )
    ...
    assert result.returncode == 0, (
        f"MOTD script should gracefully handle missing model.json: {result.stderr}"
    )
    assert "Capacity:" not in result.stdout, (
        "No capacity line should appear when model.json missing"
    )
...
def test_motd_convergence_status_badge(model_json_with_capacity):
    ...
    output_clean = (
        result.stdout.replace("\033[0;32m", "")
        .replace("\033[1;33m", "")
        .replace("\033[2m", "")
        .replace("\033[0m", "")
    )
...
def test_motd_shows_new_battery_alert(temp_model_json):
    ...
    assert "⚠️  Possible new battery detected" in result.stdout or "⚠️" in result.stdout, (
    ...
    assert "2026-03-16T10:30:00" in result.stdout, f"Timestamp not found in output: {result.stdout}"
    ...
    assert (
        "⚠️  Possible new battery detected" not in result.stdout
        and "ups-battery-monitor --new-battery" not in result.stdout

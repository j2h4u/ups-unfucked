...
def test_write_health_endpoint_creates_file(health_endpoint_temp_file, baseline_health_params):
    ...
    write_health_endpoint(
        HealthSnapshot(**baseline_health_params), health_path=health_endpoint_temp_file
    )
...
def test_health_endpoint_includes_shutdown_imminent(
    ...
    write_health_endpoint(
        ...
        health_path=health_endpoint_temp_file,
    )
    with open(health_endpoint_temp_file) as f:
        data = json.load(f)
...
def test_health_endpoint_nulls_when_optional_fields_not_provided(
    ...
    # Call without optional snapshot parameters; the destination remains explicit.
    write_health_endpoint(
        HealthSnapshot(**baseline_health_params), health_path=health_endpoint_temp_file
    )
...
def test_health_endpoint_unix_timestamp(health_endpoint_temp_file, baseline_health_params):
    ...
    with patch("src.monitor_config.time.time", return_value=fixed_time):
        write_health_endpoint(
            HealthSnapshot(**baseline_health_params), health_path=health_endpoint_temp_file
        )
...
def test_health_endpoint_journal_fields_have_safe_defaults(
    health_endpoint_temp_file, baseline_health_params
):
    """Journal observability is always present even before journal wiring is enabled."""
    write_health_endpoint(
        HealthSnapshot(**baseline_health_params), health_path=health_endpoint_temp_file
    )
    data = json.loads(health_endpoint_temp_file.read_text())

    assert data["journal_healthy"] is True
    assert data["active_event_id"] is None
    assert data["journal_last_synced_seq"] is None
    assert data["journal_last_error"] is None
    assert data["pending_replay"] is False
    assert data["recovered_partial_events"] == 0


def test_health_endpoint_serializes_journal_fields_and_redacts_error(
    health_endpoint_temp_file, baseline_health_params
):
    """Journal state is serialized without leaking paths or multiline exceptions."""
    raw_error = "/srv/private/ups/discharge-events-v1.jsonl: permission denied\n" + "x" * 1000
    write_health_endpoint(
        HealthSnapshot(
            **baseline_health_params,
            journal_healthy=False,
            active_event_id="event-123",
            journal_last_synced_seq=42,
            journal_last_error=raw_error,
            pending_replay=True,
            recovered_partial_events=3,
        ),
        health_path=health_endpoint_temp_file,
    )
    data = json.loads(health_endpoint_temp_file.read_text())

    assert data["journal_healthy"] is False
    assert data["active_event_id"] == "event-123"
    assert data["journal_last_synced_seq"] == 42
    assert data["pending_replay"] is True
    assert data["recovered_partial_events"] == 3
    assert data["journal_last_error"]
    assert "/srv/private/ups" not in data["journal_last_error"]
    assert "\n" not in data["journal_last_error"]
    assert len(data["journal_last_error"]) <= 256

...
from src.battery_math.lut import LutPoint, inverse_lut_voltage, soc_from_voltage
...
@pytest.mark.parametrize(
    ("soc", "expected"),
    ((1.1, 13.4), (-0.1, 10.5), (0.64, 12.4), (0.82, 12.9)),
)
def test_inverse_lut_voltage_clamps_and_interpolates(soc, expected):
    assert inverse_lut_voltage(soc, LUT) == pytest.approx(expected)


def test_forward_inverse_round_trip_and_total_empty_behavior():
    for soc in (0.0, 0.2, 0.64, 0.8, 1.0):
        voltage = inverse_lut_voltage(soc, LUT)
        assert soc_from_voltage(voltage, LUT) == pytest.approx(soc, abs=0.004)
    assert soc_from_voltage(12.0, ()) == 0.5
    assert inverse_lut_voltage(0.5, ()) == 0.0

...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"previous_final_record_sha256": "wrong"}, "wrong previous final record"),
        ({"previous_segment_file_sha256": "wrong"}, "file hash is invalid"),
        ({"previous_segment_file_sha256": 42}, "file hash is invalid"),
    ],
)
...
def test_gap_links_accept_matching_and_absent_optional_links() -> None:
    ...
    matching = _record(
        ...
        previous_final_record_sha256=previous.record_sha256,
        previous_segment_file_sha256="d" * 64,
    )
    _validate_gap_link(matching, previous, "d" * 64)
    _validate_gap_link(_record("gap", segment="c" * 32), previous, None)
...
def test_gap_links_reject_a_mismatched_previous_file() -> None:
    ...
    with pytest.raises(EventCorruptionError, match="wrong previous file"):
        _validate_gap_link(gap, previous, "e" * 64)
...
def test_segment_boundaries_validate_gap_links_and_damage_terminal() -> None:
    ...
    _validate_segment_boundaries(
        ((start, end), (gap, damaged)),
        segment_file_hashes=("d" * 64, "e" * 64),
    )

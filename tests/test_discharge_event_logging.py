...
def test_append_discharge_event_to_model(
    battery_model_with_discharge_events, sample_discharge_event
):
...
@pytest.mark.integration
def test_discharge_event_schema_required_fields(
    battery_model_with_discharge_events, sample_discharge_event
):
...
@pytest.mark.integration
def test_discharge_event_persisted_in_model_json(
    battery_model_with_discharge_events, sample_discharge_event
):
...
@pytest.mark.integration
def test_discharge_event_timestamp_format(
    battery_model_with_discharge_events, sample_discharge_event
):
...
@pytest.mark.integration
def test_prune_discharge_events_keeps_last_30(
    battery_model_with_discharge_events, sample_discharge_event
):
...
@pytest.mark.integration
def test_discharge_events_queryable_by_reason(
    battery_model_with_discharge_events, sample_discharge_event
):
    ...
    test_events = [
        e for e in model.state["discharge_events"] if e["event_reason"] == "test_initiated"
    ]

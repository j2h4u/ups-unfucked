...
class TestStrictListVar:
    """Tests for the complete, read-only LIST VAR boundary used by Slice 0."""

    @staticmethod
    def _reply(*lines: str) -> bytes:
        return ("\n".join(lines) + "\n").encode()

    def test_strict_success_sends_only_list_var_and_preserves_tokens(self, mock_nut_socket):
        mock_nut_socket.recv.return_value = LIST_VAR_RESPONSE.encode()
    ...
    def test_strict_rejects_non_var_line(self, mock_nut_socket):
        mock_nut_socket.recv.return_value = self._reply(
            "BEGIN LIST VAR cyberpower",
            "NOTICE unexpected protocol line",
            "END LIST VAR cyberpower",
        )
    ...
    def test_strict_rejects_duplicate_key(self, mock_nut_socket):
        mock_nut_socket.recv.return_value = self._reply(
            "BEGIN LIST VAR cyberpower",
            'VAR cyberpower ups.status "OL"',
            'VAR cyberpower ups.status "OB"',
            "END LIST VAR cyberpower",
        )

        with pytest.raises(ValueError, match="duplicate LIST VAR key"):
            NUTClient().get_ups_vars_with_tokens_strict()

    def test_strict_rejects_empty_reply(self, mock_nut_socket):
        mock_nut_socket.recv.return_value = self._reply(
            "BEGIN LIST VAR cyberpower",
            "END LIST VAR cyberpower",
        )

        with pytest.raises(ValueError, match="contains no variables"):
            NUTClient().get_ups_vars_with_tokens_strict()
